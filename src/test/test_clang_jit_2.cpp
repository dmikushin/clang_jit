...
int main(int argc, char *argv[]) {
  ...
  // Add a generator to the JIT dynamic library to resolve symbols from the
  // current process. Note: in order for the JIT dynamic library to use the
  // symbols from the current process, those symbols must be present in the
  // dynamic table. If the symbols are defined in the executable image, they
  // won't be accessible to JIT, because the dynamic table is not generated for
  // an executable by default. The solution is to link the executable with
  // --export-dynamic, or set_target_properties(target PROPERTIES ENABLE_EXPORTS
  // ON) for CMake
  ...
  ExitOnErr(
      J->addIRModule(JD, ThreadSafeModule(std::move(M.get()), std::move(C))));
